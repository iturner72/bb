...
//                        let existing_record = match &response_json {
//                            serde_json::Value::Array(arr) if !arr.is_empty() => Some(&arr[0]),
//                            _ => None
//                        };
//                        
//                        let has_openai_embedding = existing_record
//                            .and_then(|r| r.get("embedding"))
//                            .map(|e| !e.is_null())
//                            .unwrap_or(false);
//                            
//                        if has_openai_embedding {
//                            info!("Skipping '{}' - already has OpenAI embedding", post.title);
//                            company_progress.skipped_posts += 1;
//                            progress_sender.send(company_progress.clone().into_event())
//                                .await
//                                .map_err(|e| format!("Failed to send progress update: {}", e))?;
...
//                        }
//                        
//                        let record_exists = existing_record.is_some();
//
//                        if record_exists {
